...
THIS FILE DIFFERS FROM the first test in that:
    we are sampling every tenth of a second, and taking the last
    second and a half (240 samples) seconds worth of samples.
    - a tenth of a sec is 16 samples

    the methodology will follow:
        trial 1: first 240 samples
        trial 2: [16:256] samples
        trial 3: [32:272] samples
        and so on

RESULTS: 49% ACC: PROBABLY OVERFIT
...
num_labels = 5
X, Y = read_edf_files.reader('/home/kaleb/Documents/eeg_dataset/files/', 1) # use other function to read the edf files
################################################################
