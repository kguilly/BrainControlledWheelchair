...
def system_os_check():
    ...
    if (system_platform == "Linux"):
        systemInitLabel.config(text='Hello Linux user, please input ttyUSB port number for BCI headset below.')
        systemInitComLabel.config(text="/dev/ttyUSB")
        systemInitComLabel.place(x=27,y=118)
        


def system_init_continue_button():
    sysInitSpaceTest = " " in (systemInitEntry.get())
    
    if (len((systemInitEntry.get())) > 0 and len((systemInitEntry.get())) <= 3 and str(sysInitSpaceTest) == "False" and (systemInitEntry.get()).isdigit()):
        if (system_platform == "Windows"):
            headset_directory = "COM" + systemInitEntry.get()
        elif (system_platform == "Linux"):
            headset_directory = "/dev/ttyUSB" + systemInitEntry.get()
            # Add Linux COM code equivalent here if any is needed (FOR KALEB)

        print(headset_directory)
        systemInitWindow.withdraw()
        #print(str((systemInitEntry.get()).isdigit())) #== "True"
        systemInitEntry.delete(0,tk.END)
        systemInitErrorLabel.config(text="")
        startWindow.deiconify()
    else:
        if (len((systemInitEntry.get())) <= 0):
            systemInitErrorLabel.config(text="ERROR: Must contain at least one number!")
        elif (str((systemInitEntry.get()).isdigit()) == "False"):
            systemInitErrorLabel.config(text="ERROR: Must only contain numbers!")
        elif (len((systemInitEntry.get())) > 3):
            systemInitErrorLabel.config(text="ERROR: Too many numbers!")
        elif (str(sysInitSpaceTest) == "True"):
...
systemInitWindow.geometry('540x400')
...
systemInitLabel = tk.Label(systemInitWindow, 
    text='Hello Windows user, please input COM port number for BCI headset below.', 
    fg="#41FF00", 
    bg="black", 
    wraplength=440,
    font=('Terminal',18),
...
# Creates label to show word "COM"
systemInitComLabel = tk.Label(systemInitWindow, 
    text='COM', 
    fg="#41FF00", 
    bg="black", 
    font=('Terminal',18),
    ...
    padx=10,
    pady=10)
systemInitComLabel.place(x=153,y=118)

# Creates an entry box for inputting number of COM port
systemInitEntry = tk.Entry(systemInitWindow,
    font=('Terminal',19),
    relief=tk.RAISED,
    width=5,
    bd=8,
