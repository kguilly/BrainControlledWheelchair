...
def directional_window():
    ...
    #    stop_joystick()
    global keyboard_user_mode, headset, curr_file_path, start_headset_session_flag, i
    i+=1
    print(i)

    # Used for testing the functionality of IMU without implementing it yet.
    if (i > 200):
    ...
    if keyboardMode == True:
        ...
        #if not headsetMode: # if we're switching away from using the headset
        #    init.end_session(headset)
        #    headsetMode = True # flag that indicates that the 
        ...
        if headsetMode:
            print("This section was causing errors with displaying controls")
            #headsetMode = False
        ...
        prediction = "This is just placeholder because it was causing errors for testing"
...
def system_init_continue_button():
    ...
    if (len((systemInitHeadsetEntry.get())) > 0 and len((systemInitHeadsetEntry.get())) <= 3 and str(sysInitSpaceTest) == "False" 
        ...
        if (system_platform == "Windows"):
            ...
            pi_directory = "/dev/ttyUSB" + systemInitPiEntry.get()
