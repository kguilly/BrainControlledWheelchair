...
def predicted_direction(passed_model):
    global prediction, headset, profSelected, curr_file_path

    output_prediction = ml.generate_prediction(headset, passed_model)
    # output_prediction = 'forward'
...
# Used for testing frame counts of directional window (will be deleted in final version)
################################################################################################################
...
def directional_window():
    global headset, curr_file_path, \
        switch_state, prediction, flip_warning, collision_warning, profile_path, model_path, model

    headset_prediction_thread = None
    ...
    reverse_oval = canvas.create_oval(
        ((canvas_width / 2) - oval_size_left_right_reverse),
        (((canvas_height / 2) + oval_spacing) - (oval_size) * (2 / 3)),
        ((canvas_width / 2) + oval_size_left_right_reverse),
        (((canvas_height / 2) + oval_spacing) + (oval_size) * (2 / 3)),
...
def system_init_continue_button():
    ...
    if (len((systemInitHeadsetEntry.get())) > 0 and len((systemInitHeadsetEntry.get())) <= 3 and str(
            sysInitSpaceTest) == "False"
            and (systemInitHeadsetEntry.get()).isdigit() and len((systemInitPiEntry.get())) > 0 and len(
                (systemInitPiEntry.get())) <= 3 and str(sysInitSpaceTest) == "False"
        ...
        elif (str((systemInitHeadsetEntry.get()).isdigit()) == "False" or str(
                (systemInitPiEntry.get()).isdigit()) == "False"):
...
def training_complete_finish_button():
    global profile_path, model_path, model
    ...
    accuracy = ml.train_the_model(
        profile_path)  # the model is saved to the user's directory       (TEMPORARILY DISABLED FOR TESTING BY SAMUEL) **********************************************************************************************
    ...
    print(
        f'Your trained model\'s accuracy is {accuracy}')  # (TEMPORARILY DISABLED FOR TESTING BY SAMUEL) **********************************************************************************************
...
def load_profile_button(pickedProfile):
    global profSelected, profile_path, model_path, model
    if (pickedProfile == 1):
        profSelected = "1"
...
file_menu = tk.Menu(menu,
                    tearoff=False)  # the tearoff parameter is whether or not you want the default tearoff that opens a new window
